...
def main():
	...
	valid_data = CelebA(root='data', split="valid", download=False, transform=transform, target_type=["attr"])
	valid_data_loader = DataLoader(valid_data, batch_size=32, shuffle=False, num_workers=8)
	...
	model.to(device)
	with torch.no_grad():
		n_class_correct = {arg: 0 for arg in valid_data.attr_names if arg}
		attribute_metrics = {attr: {"TP": 0, "FP": 0, "FN": 0} for attr in valid_data.attr_names}
		n_samples = 0
		all_eval_preds = []
		all_eval_labels = []
		...
		for inputs, labels in tqdm(valid_data_loader):
			...
			for i, out in enumerate(outputs):
				for j, label in enumerate(labels[i]):
					if out[j] == label:
						n_class_correct[valid_data.attr_names[j]] += 1
				n_samples += 1
	...
	n_class_portion = {
		...
		for arg in valid_data.attr_names if arg}
