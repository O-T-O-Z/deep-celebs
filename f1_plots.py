...
def latex_table():
    ...
    _, _, scores_multi = read_data(os.path.join("models", "multitaskSGD", "results_attr.txt"))
    sep = " & "
    end = " \\\\"
...
def plot():
    ...
    _, scores_multi, _ = read_data(os.path.join("models", "multitaskSGD", "results_attr.txt"))

    plt.rcParams["figure.figsize"] = (15,5)
    colors =["lightskyblue", "royalblue"]
    df = pd.DataFrame({"Single Task": scores_single, "Multi Task":scores_multi},columns=['Single Task', 'Multi Task'], index = atts)
